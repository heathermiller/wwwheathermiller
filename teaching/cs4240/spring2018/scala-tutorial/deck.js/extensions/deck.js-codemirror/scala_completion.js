// needs codemirror 3
var initializeCompletion = function(CodeMirror) {
  CodeMirror.commands.autocomplete = function(cm) {
    CodeMirror.showHint(cm, CodeMirror.scalaReplHint);
  }

  // based on http://codemirror.net/addon/hint/javascript-hint.js
  var scalaKeywords = ("case catch do else false finally for if new null return match throw true try var val def type while with trait class extends").split(" ");

  CodeMirror.scalaReplHint = function(editor, options) {
    function getScalaReplCompletions(token, context, keywords, options)       {
      var found = [], start = token.string;
      function maybeAdd(str) {
        if (str.indexOf(start) == 0 && !arrayContains(found, str)) found.push(str);
      }

      // TODO: contact repl with line info and add results
    ...
    function scriptHint(editor, keywords, getToken, options) {
      ...
      if (!/^[\w$_]*$/.test(token.string)) {
        token = {start: cur.ch, end: cur.ch, string: "",
                 state: token.state, type: token.string == "." ? "property" : null};
      }

      return {list: getScalaReplCompletions(token, context, keywords, options),
              from: Pos(cur.line, token.start),
              to: Pos(cur.line, token.end)};
    }

    return scriptHint(editor, scalaKeywords,
                      function (e, cur) {return e.getTokenAt(cur);},
                      options);
  };

  //////////////////////
  // http://codemirror.net/addon/hint/show-hint.js
